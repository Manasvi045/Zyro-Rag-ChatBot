...
# Initialize session state for query injections from sidebar buttons
...
with st.sidebar:
    ...
    # ENHANCEMENT 1: Clickable Sample Question Buttons
...
for msg in st.session_state.messages:
    with st.chat_message(msg["role"]):
        ...
        # Persist static performance metadata if present
...
# -------------------
# Capture user input from either the regular text bar OR a clicked sidebar button
raw_input = st.chat_input("Ask a question about HR policies...")
question = raw_input or st.session_state.sample_query

if question:
    # Immediately clear the state tracking the button click so it doesn't loop
    st.session_state.sample_query = None
    ...
    with st.chat_message("assistant"):
        with st.spinner("Searching company policies..."):
            ...
            # ENHANCEMENT 2: Latency Benchmark Tracker
            ...
            # ENHANCEMENT 3: Render Production Telemetry
            meta_text = f"⚡ Latency: {execution_time}s | 🎯 Confidence: High | 🔍 System: RAG-V1"
            ...
            # FIX: Native, perfectly aligned thumbs up/down feedback widget
            feedback = st.feedback("thumbs", key=f"feedback_{time.time()}")
            if feedback is not None:
                # Handle feedback logging if needed (0 for downvote, 1 for upvote)
                pass
            
    # Save clean string & metadata into state history
    st.session_state.messages.append(
        {
            "role": "assistant",
